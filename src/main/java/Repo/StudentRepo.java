...
public class StudentRepo implements ICrudRepository<Student>{
    ...
    public Student findStudent(String LastName,String FirstName){
        try (Connection con = DriverManager
            ...
            statement=con.createStatement();
            ResultSet resultSet = statement.executeQuery(Query);
            Student myStudent = new Student("","",0,0,null);
            if(resultSet!=null){
                myStudent.setStudentId(resultSet.getInt("StudentId"));
                myStudent.setLastname(resultSet.getString("LastName"));
                myStudent.setTotalCredits(resultSet.getInt("Credits"));
                return myStudent;
            }
            else{
